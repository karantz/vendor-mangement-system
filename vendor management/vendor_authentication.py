...
import time
import random
import vendor_inventory as ven
...
c.execute("CREATE table if not existS vendor_au(NAME VARCHAR(30),CONTACT VARCHAR(30),EMAIL VARCHAR(30),PASSWORD VARCHAR(30),GENDER VARCHAR(30),CITY VARCHAR(30),STATE VARCHAR(30))")
def reg():
    def record():
        name1=name.get()
        con1=contact.get()
        ...
        if name1=='' or con1==''or email1=='' or gen1==''or city1==''or state1==''or pass1=='':
           ...
           insert_stmt = (
               "INSERT INTO vendor_au(NAME, CONTACT, EMAIL, PASSWORD, GENDER, CITY, STATE)"
...
def login():
    def verify():
        ...
        if email1=='' or password1=='':
            ...
            q="select password from vendor_au where email='"+email1+"'"
            c.execute(q)
            p=('',)
            for i in c:
                p=i
